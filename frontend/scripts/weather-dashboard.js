...
// =========================================
// PREFILL FROM FARMER PROFILE & SAVED LOCATION
// =========================================

async function prefillFromProfile() {

    let stateVal = localStorage.getItem("farmer_state") || "";
    let districtVal = localStorage.getItem("farmer_district") || "";
    ...
    if (!stateVal || !districtVal) {
        ...
        if (saved) {
            try {
                ...
                if (profile.state) stateVal = profile.state;
                if (profile.district) districtVal = profile.district;
            } catch (e) {}
        }
    }
    ...
    try {
        ...
        const match = options.find(
            opt => opt.value.toLowerCase() === stateVal.toLowerCase()
        );

        if (match) {
            stateSelect.value = match.value;
            await loadDistrictsForState(match.value, districtVal || "");

            // If both state and district are ready, auto-fetch weather immediately
            if (districtSelect.value && cropSelect.value) {
                const fakeEvent = { preventDefault: () => {} };
                handleGetWeather(fakeEvent);
            }
        }
    } catch (e) {
        console.warn("Prefill location error:", e);
    }
