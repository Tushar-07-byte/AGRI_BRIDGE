...
(function () {
    ...
    const AdminApp = {
        ...
        checkBackendHealth: function () {
            ...
            const dbPill = document.getElementById("db-runtime-status-pill");
            const dbText = document.getElementById("db-status-text");
            const dbDot = document.getElementById("db-status-dot");

            // Database Runtime Dialect Status
            fetch(`${API_BASE_URL}/api/db/status`)
                .then(res => res.json())
                .then(data => {
                    if (data && data.success && dbText && dbDot && dbPill) {
                        if (data.is_fallback) {
                            dbText.textContent = "DATABASE: SQLite FALLBACK — ACTIVE";
                            dbPill.style.background = "rgba(239, 68, 68, 0.2)";
                            dbPill.style.color = "#ef4444";
                            dbPill.style.border = "1px solid rgba(239, 68, 68, 0.4)";
                            dbDot.style.background = "#ef4444";
                            dbPill.title = data.warning || "SQLite Fallback Active";
                        } else {
                            dbText.textContent = "DATABASE: MySQL — ACTIVE";
                            dbPill.style.background = "rgba(34, 197, 94, 0.15)";
                            dbPill.style.color = "#22c55e";
                            dbPill.style.border = "1px solid rgba(34, 197, 94, 0.3)";
                            dbDot.style.background = "#22c55e";
                            dbPill.title = `MySQL Active (${data.database_name})`;
                        }
                    }
                })
                .catch(err => console.warn("Admin DB status fetch notice:", err));
