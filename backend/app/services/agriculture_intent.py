...
def normalize_digits(text: str) -> str:
    """Normalize Odia, Devanagari, and Gurmukhi numerals to standard ASCII digits."""
    if not text:
        return ""
    odia_digits = str.maketrans("୦୧୨୩୪୫୬୭୮୯", "0123456789")
    dev_digits = str.maketrans("०१२३४५६७८९", "0123456789")
    gur_digits = str.maketrans("੦੧੨੩੪੫੬੭੮੯", "0123456789")
    return text.translate(odia_digits).translate(dev_digits).translate(gur_digits)


def get_stage_from_days(days: int) -> str:
    """Infer growth stage from days after sowing."""
    if days < 15:
        return "Germination"
    elif days <= 45:
        return "Vegetative"
...
def extract_crop_entity(text: str) -> Optional[str]:
    """Extract crop name from utterance using alias mapping."""
    if not text:
        return None
    normalized = normalize_digits(text).lower()
    odia_crops = {
        "ଗହମ": "Wheat", "ଧାନ": "Rice", "ମକା": "Maize",
        "ଟମାଟୋ": "Tomato", "ଆଳୁ": "Potato", "କପା": "Cotton",
        "ପିଆଜ": "Onion", "ଲଙ୍କା": "Chilli", "ଆଖୁ": "Sugarcane",
        "ସୋରିଷ": "Mustard", "ସୋୟାବିନ": "Soybean"
    }
    for o_crop, c_crop in odia_crops.items():
        if o_crop in text:
            return c_crop
...
def extract_crop_age_days(text: str) -> Tuple[Optional[int], Optional[str]]:
    """Extract age in days and infer stage."""
    if not text:
        return None, None
    norm = normalize_digits(text)
    m = re.search(r"(\d{1,3})\s*(din|day|days|ଦିନ|दिन|ਦਿਨ|दिवस)", norm, re.IGNORECASE)
    if not m:
...
def extract_location(text: str) -> Optional[Dict[str, Any]]:
    """Extract district location from utterance."""
    try:
        from app.services.india_districts import search_district_by_name
        return search_district_by_name(text)
    except Exception:
        try:
            from app.data.india_districts import search_district_by_name
            return search_district_by_name(text)
        except Exception:
            return None


def is_short_followup_answer(text: str, pending_field: Optional[str] = None) -> bool:
    """Check if utterance is a brief follow-up answering a pending prompt."""
    if not text:
        return False
